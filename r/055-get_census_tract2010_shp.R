## r/055-get_census_tract2010_shp.R
...
output_dir <- here::here("census-tract-shp/2010/")
mkdir_p(output_dir)

f_dwnld_shp <- function(st) {

  tmp <- tigris::tracts(state = st, year = 2010)
...
f_census_dwnld <- function(st) {
  ...
  dt_sf <- tigris::tracts(state = st, year = 2010) %>%
...
saveRDS(dt_all, here::here("census-tract-shp/census_tract2010_shp.rds"))
