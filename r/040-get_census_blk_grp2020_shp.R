## r/040-get_census_blk_grp2020_shp.R
...
output_dir <- here::here("census-blk-grp-shp/2020/")
mkdir_p(output_dir)

f_dwnld_shp <- function(st) {

  tmp <- tigris::block_groups(state = st, year = 2020)
...
f_census_dwnld <- function(st) {
  ...
  dt <- get_decennial(geography = "block group", variables = c("H1_001N", "P1_001N"),
                      year = 2020, state = st) %>%
    as.data.table()
  ...
  dt_sf <- tigris::block_groups(state = st, year = 2020) %>%
...
saveRDS(dt_all, here::here("census-blk-grp-shp/census_blk_grp2020_shp.rds"))
